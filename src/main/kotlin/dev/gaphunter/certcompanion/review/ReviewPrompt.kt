package dev.gaphunter.certcompanion.review
 ...
 * number of successfully decoded certificates/keystores -- never on
 * install, never on a timer, and never for a parse failure or a wrong
 * keystore password. Counts invocations directly instead of tracking a
 * set of seen keys -- a passive viewer has no per-line "finding" to
 * dedupe, just the fact that it successfully showed the user something
 * real.
 *
...
object ReviewPrompt {
    ...
    /** How many successful decodes before the prompt shows once. */
    private const val HITS_BEFORE_PROMPT = 5

    private const val KEY_HIT_COUNT = "dev.gaphunter.certcompanion.review.hitCount"
    private const val KEY_ANSWERED = "dev.gaphunter.certcompanion.review.answered"

    private const val NOTIFICATION_GROUP_ID = "Cert Companion"

    private const val MARKETPLACE_URL = "https://plugins.jetbrains.com/plugin/33153-cert-companion/reviews"

    /**
     * Call this once per real, successful certificate/keystore decode
     * (not for a parse failure or a wrong password). Safe to call on
     * any thread.
     */
